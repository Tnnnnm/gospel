...
class Testify{
	...
	public function report(){
		...
		foreach($cases as $caseTitle => $case){
					if($case['fail'] ==0){
						$this->succe_mes("bummer,everything passed!");
						}else{
						$this->error_mes("far out,a failing tests!");
						}
						echo "TestCaseName->".$caseTitle."\n";
						$this->succe_mes($case['pass']." cases passed");
						$this->error_mes($case['fail']." cases failed");
						
		foreach ($case['tests'] as $test){
						if($test['result'] == 'pass'){
							$this->succe_mes("result: ".$test['result'].", assertType: ".$test['type'].", linein: ".$test['line'].", filein: ".$test['file']);
						}else{
							$this->error_mes("result: ".$test['result'].", assertType: ".$test['type'].", linein: ".$test['line'].", filein: ".$test['file']);
						}
		}
	}
	...
	function error_mes($string) {  
		$cmd="printf \"\033[31m".$string." \033[0m\n\"";
		$a=exec($cmd);
		print "$a"."\n";
	}  
	  
	function succe_mes($string) {  
